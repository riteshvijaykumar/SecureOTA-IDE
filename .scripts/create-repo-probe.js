(async()=>{
  try{
    ...
    const repo='secureota-e2e-create-' + Date.now();
    const headers=await s.getAuthHeaders();
    const resp=await fetch('https://api.github.com/user/repos',{method:'POST',headers,body:JSON.stringify({name:repo,private:false})});
