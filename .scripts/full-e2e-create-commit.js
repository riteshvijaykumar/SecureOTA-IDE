(async()=>{
  try{
    ...
    const repo='secureota-e2e-initialize-' + Date.now();
    console.log('owner',owner,'repo',repo);
    const headers=await s.getAuthHeaders();
    // create repo
    let resp=await fetch('https://api.github.com/user/repos',{method:'POST',headers,body:JSON.stringify({name:repo,private:false})});
    console.log('create status',resp.status); console.log(await resp.text());
    // create initial README to have a commit
    const readmeContent = Buffer.from('# SecureOTA E2E\n').toString('base64');
    const createReadmeResp = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/README.md`,{method:'PUT',headers,body:JSON.stringify({message:'Initial commit',content:readmeContent})});
    console.log('create readme status',createReadmeResp.status); console.log(await createReadmeResp.text());
    // create release
    ...
    // upload asset if release created
    try{
      ...
      if(release.upload_url){
        ...
        const sampleBin = path.join(sampleDir,'probe2.bin'); fs.writeFileSync(sampleBin,Buffer.from('data'));
